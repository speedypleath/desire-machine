...
- **agent.py**: `DesireAgent` wraps Ollama. Builds dynamic system prompts injecting current state percentages and commentary. Keyword-based `classify_interaction()` maps user input to `InteractionType`. `should_refuse()` gates responses based on detachment/desire levels. Post-processes responses for conciseness (1–3 sentences).
